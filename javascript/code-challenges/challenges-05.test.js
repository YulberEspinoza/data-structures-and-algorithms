...
const hasChildrenValues = (arr, character) => {
  ...
  for (let char of arr) {
    if (char.name === character) {
      return Object.values(char).some(
...
const hasChildrenEntries = (arr, character) => {
  // Solution code here...
  return Object.entries(arr).some(
    ([key, item]) =>
      item.name === character && item.children && item.children.length > 0
  );
};
...
const totalCharacters = (arr) => {
  ...
  arr.forEach((character) => {
    ...
    if (character.children) count += character.children.length;
  });
  return count;
};
